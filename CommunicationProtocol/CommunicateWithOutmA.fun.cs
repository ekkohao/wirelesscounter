...
namespace CommunicationProtocol
{
    ...
    /// 无全电流协议.重写基类方法
    /// </summary>
    partial class CommunicateWithOutmA:ProtoColConnect
    {
        ...
        public  override byte[] CopyMsg(int startbit, int endbit, byte[] copy)
        {
            ...
            byte[] temp = new byte[endbit - startbit+2];
        ...
        /// <param name="temp"></param>
        /// <returns></returns>
        public override byte[] WriteMsg(byte[] temp, int time, int timedata)
        {
            byte[] _Msg = new byte[10];
            _Msg[0] = 0x11;//协议头为0x11
            for (int i = 0; i < 4; i++)
            ...
            _Msg[6] = Convert.ToByte(time); //次数高位,次数低位
        ...
        //public override byte[] Back_HistroyMsg()
        //{
        //    byte[] temp = new byte[4];
        ...
        //public override byte[] Back_HistroyCount()
        //{
        //    byte[] temp = new byte[4];
